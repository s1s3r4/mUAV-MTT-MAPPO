...
class MADDPGPolicy(MLPPolicy):
    ...
    def get_actions(self, obs, available_actions=None, t_env=None, explore=False, use_target=False, use_gumbel=False):
        ...
        if self.discrete:
            ...
            if explore:
                # temp = torch.tensor(data=self.args.act_noise_std, device=self.device)
                actions = gaussian_noise(actor_out.shape, self.args.act_noise_std, device=self.device) + actor_out
            elif use_target and self.target_noise is not None:
                assert isinstance(self.target_noise, float)
                actions = gaussian_noise(actor_out.shape, self.target_noise, device=self.device) + actor_out
            else:
                actions = actor_out
