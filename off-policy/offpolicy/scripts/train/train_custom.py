...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            from offpolicy.envs.env_continuous import ContinuousActionEnv
...
def make_eval_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            env = ContinuousActionEnv(render=True, num=rank)
...
def parse_args(args, parser):
    ...
    parser.add_argument('--num_agents', type=int,
                        default=temp.agent_num, help="number of agents")
...
def main(args):
    ...
    if all_args.algorithm_name in ["rmatd3", "rmaddpg", "rmasac", "qmix", "vdn"]:
        ...
        from offpolicy.runner.mlp.custom_runner import MPERunner as Runner
