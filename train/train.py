...
def make_eval_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            env = ContinuousActionEnv(fixed=False, render=True, num=rank)
            # from envs.env_discrete import DiscreteActionEnv
            # env = DiscreteActionEnv()
            env.seed(all_args.seed + rank * 1000)
            return env
...
def parse_args(args, parser):
    ...
    temp = EnvCore()
    parser.add_argument("--scenario_name", type=str, default="UAV_MTT", help="Which scenario to run on")
    # parser.add_argument("--num_landmarks", type=int, default=3)
    parser.add_argument("--num_agents", type=int, default=temp.agent_num, help="number of players")
